...
#       THEN( '(?y) is a penguin' ) ) )
##
p1=(IF( AND( 'parent (?x) (?y)',  # Son
            'male (?y)' )),
THEN( '(?y) is the son of (?x)' ))

p2=(IF(AND('parent (?x) (?y)',  # daughter
       'female (?y)')),
THEN('(?y) is the daughter of (?x)'))

p3=(IF(AND('parent (?x) (?y)',  # father
       'male (?x)')),
THEN('(?x) is the father of (?y)'))

p4=(IF(AND('parent (?x) (?y)',  # mother
       'female (?x)')),
THEN('(?x) is the mother of (?y)'))

p5=(IF(AND('parent (?x) (?y)',  # siblings
       'parent (?x) (?z))')),
THEN('siblings (?z)(?y)'))
...
# family_rules.
family_rules = [p1,p2,p3,p4,p5]                    # fill me in
